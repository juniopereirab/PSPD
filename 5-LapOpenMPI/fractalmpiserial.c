...
#define OUTFILE "out_julia_serial_mpi.bmp"
...
int main(int argc, char *argv[]) {
    ...
    MPI_Gather(pixel_array, subarea, MPI_CHAR, pixel_receive, subarea, MPI_CHAR, 0, MPI_COMM_WORLD);
    ...
    if (meurank == 0) {
        ...
        fwrite(pixel_receive, sizeof(unsigned char), area, output_file);
        fclose(output_file);
        free(pixel_array);
        free(pixel_receive);
    }
